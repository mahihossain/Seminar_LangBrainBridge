...
def get_mean_prediction(target_subject, source_subjects, voxels_dict={}):
    ...
    for source_subject in source_subjects:
        loaded = np.load('./prediction_results_harrypotter/noise_ceiling/predict_{}_with_{}_{}pcs.npy'.format(target_subject, source_subject, voxels_dict[source_subject]) ,allow_pickle=True)
        corrs.append(loaded.item().mean(0))
...
if __name__ == "__main__":
    ...
    np.save('./prediction_results_harrypotter/noise_ceiling/subject_{}_kernel_ridge.npy'.format(target_subject),np.array(ceilings))
